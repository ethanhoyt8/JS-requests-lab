...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/dog`)
    .then((res) => {
...
const newQuery = () => {
    axios.get(`http://localhost:3000/query-test?watermelon=fruit`)
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
    ...
    axios.get(`http://localhost:3000/query-test?apple=fruit`)
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
...
function createFoodItem(food) {
    ...
    foodItem.innerHTML = `<p>${food}</p>`

    document.querySelector('form').appendChild(foodItem)
}
